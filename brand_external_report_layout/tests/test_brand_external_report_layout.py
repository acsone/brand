...
class TestBrandExternalReportLayout(TransactionCase):
    def setUp(self):
        ...
        self.external_layout_view = self.env.ref("web.external_layout_standard")
        self.report_layout = self.env["report.layout"].create(
            {
                "name": "Test Layout",
                "view_id": self.external_layout_view.id,
            }  # Access the id
        )
        self.paperformat = self.env["report.paperformat"].create(
